...
export function activeDropZones(o: ZoneActivityInput): Set<DropZone> {
  ...
  // Веер не раскрыт — работают обычные боксы стола. Колода карт не принимает никогда
  // (в игре она закрыта), поэтому её в живых нет.
  if (!o.boardFan) return new Set<DropZone>(["center", "hand", "discard"]);

  // Карту вытащили из веера — ей нужно куда-то деться, и стол снова открыт весь.
  if (o.source === "board") return new Set<DropZone>(["center", "hand", "discard"]);

  // Карту тащат из руки: положить её можно только в сам веер (он лежит по центру) или
  // в сброс. Остальное закрыто — иначе карта уедет в стопку, с которой сейчас работают.
  if (o.source === "hand") return new Set<DropZone>(["center", "discard"]);
