...
/**
 * Масштаб стопки на столе.
 *
 * В РАЗДАЧЕ колода лежит одна посреди стола, она главный предмет — и рисуется крупнее
 * карты в руке. В ИГРЕ стопок две (колода и сброс), они разъехались по своим слотам и
 * работают как ярлыки: там размер обычный, карта в карту с рукой. Крупным в игре бывает
 * только раскрытый веер (см. boardFanCardScale).
 */
export function deckScale(compact = false): number {
  return compact ? 1 : anim.deck.centerScale;
}
