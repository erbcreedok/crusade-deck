...
// Убрать карту из колоды. Нужна для ОПТИМИСТИЧНОГО показа: карта, которую утащили с
// колоды пальцем, обязана исчезнуть из стопки сразу — иначе она остаётся верхней, а к
// верхней привязаны и кирпич колоды, и её тень, и вся стопка «переезжает» под палец.
export function withoutCard(order: readonly string[], card: string): string[] {
  return order.filter((c) => c !== card);
}
