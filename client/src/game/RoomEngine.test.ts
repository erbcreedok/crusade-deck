...
describe("RoomEngine: перекидывание карт между боксами", () => {
  ...
  // Пока раскрыт веер доски, ИГРОВАЯ ЗОНА отключена — центр занят самим веером. Дроп карты
  // из сброса в центр в этом состоянии = мимо боксов: карта молча возвращается, move нет.
  it("при открытом вееере дроп в игровую зону отключён — move_card не шлётся", async () => {
    const { engine, app } = await inGame();
    engine.setDiscard(["2♣", "3♣", "4♣"]);
    engine.setBoardFan("discard");
    ...
    expect(moves).toHaveLength(0); // в play не ушло — зона выключена веером
    // resolveDrop в центре при открытом вееере не даёт play (мимо боксов).
    expect((engine as any).resolveDrop(cz.cx, cz.cy, "discard")).toBeNull();
  });
...
describe("RoomEngine: веер руки — глиссандо и тени", () => {
  ...
  // Нажатие на СТЫК двух карт: ближе центр нижней (левой), но сверху лежит правая — её и
  // надо поднять. Раньше брался ближайший центр, поднималась не та карта.
  it("нажатие на перекрытие поднимает верхнюю (правую) карту, а не нижнюю", async () => {
    const { engine, app } = await focusedHand(["2♦", "3♦", "4♦"]);
    const xs = handXs(engine);
    const mid = (xs[0]! + xs[1]!) / 2; // на стыке карт 0 и 1, ближе к центру нижней (0)
    // Ближайший по центру дал бы карту 0 — проверяем, что это именно «не та» карта.
    expect((engine as any).nearestHandFanIndex(mid)).toBe(0);
    pressHand(app, mid);

    const drag = (engine as any).cardDrag;
    expect(drag).not.toBeNull();
    expect(drag.v).toBe((engine as any).hand[1]); // поднялась ВЕРХНЯЯ карта стыка
  });
