...
`.github/workflows/ci.yml` гоняет тесты обоих пакетов на каждый пуш, а на `main` выкатывает
тем же `scripts/deploy.sh` — порядок выкатки и номер сборки живут в одном месте, а не в
двух расходящихся.

Две вещи, которые workflow обязан делать правильно, и обе легко упустить:

- `fetch-depth: 0` у checkout. Номер сборки — это число коммитов, а мелкий клон по
  умолчанию сделал бы его вечной единицей.
- `cancel-in-progress: false`. Отменённый на середине flyctl оставит аппу в промежуточном
  состоянии, поэтому запуски встают в очередь, а не вытесняют друг друга.

Деплою нужен секрет `FLY_API_TOKEN` в репозитории (Settings → Secrets and variables →
Actions). Создаётся командой `fly tokens create deploy`.
