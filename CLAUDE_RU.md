...
cd client && npm test && npx tsc --noEmit   # 573 теста
...
- `RoomEngine.ts` — всё ещё ~3500 строк на ~164 метода (в среднем 21 строка), и это уже
  не остатки листовой логики: осталась связная сердцевина над ~120 приватными полями —
  `onTick` (228 строк, трогает все анимации), `mount` (221), автомат жестов
  (`onPointerMove`/`onDeckDown`/`onPointerUp`/`dropCard`, вместе ~410) и 44 сеттера.
  Сжать его дальше можно только сменой дизайна, а не переносом: общий `CardPile`,
  анимации как объекты со `step(dt)` (тогда `onTick` станет циклом по списку) и жесты
  явным автоматом. Сетка безопасности под эту работу уже есть
  (`RoomEngine.test.ts` + фейк Pixi).
