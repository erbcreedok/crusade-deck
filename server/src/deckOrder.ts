...
// Является ли `next` перестановкой `cur` (тот же мультимножество карт той же длины).
// Порядок колоды теперь считает КЛИЕНТ (чтобы анимация была точной и мгновенной), а
// сервер принимает готовый порядок — но обязан проверить, что карты не подменили.
