...
// Снять ВЕРХНЮЮ карту колоды — так игрок тянет её себе в режиме свободы. Верх колоды
// это последний элемент массива (та же договорённость, что у клиентского topCard.ts).
//
// Конфликт двух одновременных «тянучек» разруливается сам собой: функция чистая и берёт
// ровно одну карту, а сообщения комнаты Colyseus обрабатывает по очереди — кто пришёл
// первым, тот и получил верхнюю, второму досталась следующая (или null на пустой).
export function takeTopCard(deck: readonly string[]): DealResult | null {
