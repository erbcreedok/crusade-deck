...
// Одна кучка карт в игральной зоне. Отдельный класс, а не массив массивов, потому что
// @colyseus/schema умеет синкать только массив СХЕМ — вложенный массив строк ему не
// объяснить. Верх кучки — последний элемент (как у колоды и сброса).
export class PlayStack extends Schema {
  @type({ array: "string" }) cards = new ArraySchema<string>();
}
...
export class GameState extends Schema {
  ...
  // ИГРАЛЬНАЯ ЗОНА — средний бокс стола: список отдельных кучек, в каждой от одной карты
  // и сколько угодно. Всё в ней лежит ЛИЦОМ ВВЕРХ: карту выложили на общий стол, прятать
  // нечего. Геометрии здесь нет и быть не должно — где какая кучка окажется на экране,
  // решает клиент по её индексу (см. client/src/game/playGrid.ts); сервер хранит только
  // состав. Пустых кучек в списке не бывает: опустевшая исчезает (см. playRules.ts).
  @type({ array: PlayStack }) play = new ArraySchema<PlayStack>();
