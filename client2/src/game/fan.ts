...
// Какую карту веера «схватил» палец. Карты идут слева направо и перекрываются, ПРАВАЯ
// лежит сверху, поэтому в каждой точке видна самая правая карта, чей левый край уже
// пройден. Её и берём — ту, что реально под пальцем, а НЕ ближайшую по центру: на стыке
// двух карт ближе центр нижней, и палец поднимал не ту карту.
//
// halfWidth — половина «толщины» карты для попадания. Для тача его чуть увеличивают: палец
// накрывает пятно, точка касания может упасть на стык с нижней картой, хотя метили в
// верхнюю, — расширенный левый край верхней ловит такой промах (крючок для толстых пальцев).
//
// xs считаются возрастающими (порядок веера сохраняется даже при раздвиге).
export function pickTopFanCard(xs: readonly number[], x: number, halfWidth: number): number {
  let pick = 0;
  for (let i = 0; i < xs.length; i++) {
    if (x >= xs[i]! - halfWidth) pick = i; // самая правая карта, чей левый край уже прошли
  }
  return pick;
}
