...
describe("deckScale", () => {
  it("в раздаче колода крупнее карты в руке — она одна и главная на столе", () => {
    expect(deckScale()).toBeGreaterThan(1);
  });

  it("в игре стопки обычного размера: они ярлыки своих слотов, а не главный предмет", () => {
    expect(deckScale(true)).toBe(1);
  });
});
