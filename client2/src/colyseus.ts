...
import { retryJoin } from "./retryJoin";
...
export async function joinRoom(opts: {
  ...
  return retryJoin(() => client.create("card_room", opts));
}
...
export async function createTestRoom(opts: {
  ...
  return retryJoin(() => client.create("test_room", opts));
}
...
export async function joinRoomById(
  ...
  return retryJoin(() => client.joinById(roomId, opts));
}
...
export async function joinByInviteCode(
  ...
  // Первый HTTP-запрос заодно будит спящий сервер (см. retryJoin) — поэтому и он с повтором.
  const res = await retryJoin(() => fetch(`${HTTP_URL}/rooms/by-code/${code}`));
